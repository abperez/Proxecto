#ifndef GSEA_READ_DATA_FILES
#define GSEA_READ_DATA_FILES
...
template <class exprs_t, class index_t>
  ...
  //  TIMERSTART(host_read_gct_file);
    ...
    if (gctfile.is_open()) {
        ...
        if (token != "#1.2") {
            ...
            exit(GSEA_INCORRECT_GCT_HEADER_ERROR);
        ...
        for (index_t gene = 0; gene < num_genes; gene++) {
            ...
            if(gene_symbol_set.find(gsymb[gene]) == gene_symbol_set.end()) {
                ...
                std::cout << "Error: duplicate symbol " << gsymb[gene]
                          << " , exiting." << std::endl;
                exit(GSEA_DUPLICATE_GENE_SYMBOL_ERROR);
            }
        ...
        std::cout << "Error: Unable to open file " << filename 
                  << " , exiting." << std::endl;
        exit(GSEA_CANNOT_OPEN_FILE_ERROR);
    }
...
template <class label_t, class index_t>
void read_cls(const std::string filename,
    ...
    if (clsfile.is_open()) {
        ...
        if (num_classes != 2) {
            ...
            exit(GSEA_UNSUPPORTED_NUMBER_OF_PHENOTYPES_ERROR);
        ...
        if (dummy != 1) {
            ...
            exit(GSEA_INCORRECT_CLS_HEADER_ERROR);
        ...
        for (const auto& x : symbolic_labels) {
            ...
            if (x == class_one) 
                ...
                std::cout << "ERROR: Unknown label in cls file " << filename
                          << " : \"" << x << "\", exiting." << std::endl;
                exit(GSEA_UNKNOWN_LABEL_IN_CLS_FILE_ERROR);
        ...
        if (num_type_A + num_type_B != num_patients) {
            ...
            exit(GSEA_NON_SPECIFIED_CORRUPTION_IN_CLS_FILE_ERROR);
